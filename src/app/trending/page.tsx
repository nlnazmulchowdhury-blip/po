...
import { TrendingUp, Film, SearchX } from 'lucide-react'
...
export default function TrendingPage() {
  ...
  const trendingQuery = React.useMemo(() => {
    if (!db) return null;
    return query(collection(db, 'movies'), orderBy('views', 'desc'), limit(20));
  }, [db]);

  const { data: movies, loading } = useCollection(trendingQuery);
          ...
          <div className="max-w-[2400px] mx-auto">
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-2 text-primary">
                ...
                <span className="text-xs font-bold uppercase tracking-[0.2em]">What's Hot</span>
              </div>
              ...
              <p className="text-muted-foreground max-w-2xl text-sm md:text-base font-medium">
                The most watched and talked about movies on CinemaStream today.
              </p>
            </div>
                    ...
                    <div className="col-span-full text-center py-24 border border-dashed border-white/10 rounded-3xl bg-white/[0.02]">
                      {searchQuery ? (
                        <>
                          ...
                          <p className="text-muted-foreground font-bold uppercase tracking-widest text-xs">No matching trending movies found.</p>
                        </>
                      ) : (
                        <>
                          <Film className="h-12 w-12 text-white/10 mx-auto mb-4" />
                          <p className="text-muted-foreground font-bold uppercase tracking-widest text-xs">No trending movies found.</p>
                        </>
                      )}
                    </div>
                  )}
